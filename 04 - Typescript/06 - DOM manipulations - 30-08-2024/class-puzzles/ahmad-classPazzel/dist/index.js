...
function getMoviesWithRatingAbove(rating) {
    return movies.filter(function (movie) { return movie.rating >= rating; });
}
function mainMovie() {
    try {
        var movieList = document.querySelector('#movie');
        if (!movieList)
            throw new Error('Movie list not found');
        var highRatingMovies = getMoviesWithRatingAbove(7);
