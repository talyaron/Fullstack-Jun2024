...
function getUsersCourses() {
    ...
    if (!userCoursesString)
        return [];
    return JSON.parse(userCoursesString);
}
function getCoursesByUser(userId) {
    var userCourses = getUsersCourses();
    return userCourses.filter(function (uc) { return uc.userId === userId; }).map(function (uc) { return getCourses().find(function (c) { return c.id === uc.courseId; }); });
}
function getUsersByCourse(courseId) {
    var userCourses = getUsersCourses();
    return userCourses.filter(function (uc) { return uc.courseId === courseId; }).map(function (uc) { return getUsers().find(function (u) { return u.id === uc.userId; }); });
}
function getCurrUser() {
    try {
        ...
        var user = getUsers().find(function (user) { return user.id === userString_1; });
        if (!user)
...
function renderExpandCourse(courseId) {
    try {
        ...
        if (courseElement.classList.contains('selected')) {
            ...
            courseElement.innerHTML = "\n                    " + course.name + "\n                    <table class=\"students\">\n                        <tr>\n                            <th>Name</th>\n                            <th>Email</th>\n                            <th>Phone</th>\n                        </tr>\n                        " + getUsersByCourse(course.id).map(function (user) { return "\n                            <tr>\n                                <td>" + user.name + "<td>\n                                <td>" + user.email + "<td>\n                                <td>" + user.phone + "<td>\n                            </tr>\n                        "; }).join('') + "\n                    </table>\n                ";
        }
    }
    catch (error) {
        console.log(error);
...
function addCourses() {
    ...
    if (!coursesString) {
        ...
        JSON.parse(coursesString).forEach(function (course) { return courses.push(course); });
    }
...
function ifUserCourse(courseId) {
    ...
    return usersCourses.some(function (uc) { return uc.courseId === courseId && uc.userId === user.id; });
}
function toggleUserCourse(courseId) {
    try {
        ...
        if (ifUserCourse(courseId)) {
            userCourses.splice(userCourses.findIndex(function (uc) { return uc.courseId === courseId && uc.userId === user_1.id; }), 1);
        }
        else {
            userCourses.push({ userId: user_1.id, courseId: courseId });
        }
