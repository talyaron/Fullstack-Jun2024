...
function renderSelectedCourse(event) {
    try {
        ...
        if (selected_1.length > 0) {
            resultDiv.innerHTML = "<h3>Your selected course is:<h3><ul>" + selected_1.map(function (course) { return "<li>" + course + "</li>"; }).join('') + "</ul>";
        }
        else {
...
function renderZoomPage(event) {
    try {
        ...
        if (!zoomPage)
            throw new Error('Error: Cannot find the dashboard element.');
