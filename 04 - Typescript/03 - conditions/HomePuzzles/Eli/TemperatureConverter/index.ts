const userTemp = Number(prompt("enter a temparture degrees"));
const userFC = String(
  prompt(`enter a temparture scale unit ("C" for Celcius "F" for Fahrenheit)`)
);

function tempConverter(temNum: number, temUnit: string): number {
...
const resConvert = tempConverter(userTemp, userFC);

const resRecommended = tempRecommend(userTemp, userFC, resConvert);
