...
let Temperature: number = Number(prompt("Enter a password at least 8 characters long and no more than 20 characters long."));
let Unit:string = prompt("Please enter a unit: F or C")
console.log(Temperature);
console.assertlog(Unit)

function convertingAndSuggesting(Temperature: number, Unit:string): string {
  if (Unit === "C") {
...
const result = convertingAndSuggesting(Temperature, Unit);
document.write(result);
