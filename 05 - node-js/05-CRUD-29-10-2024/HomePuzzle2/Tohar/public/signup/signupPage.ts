...
function renderSignupPage() {
    ...
    `
    const signupPageElement = document.querySelector<HTMLDivElement>('#signupPage');
    if (!signupPageElement) throw new Error('Login page not found');
...
async function handleFormRegister(event: Event) {
        ...
        const pswConfirm = (document.getElementById('pswConfirm') as HTMLInputElement).value;
    ...
    async function userExists(email: string) {
        try {
            const response = await fetch(`http://localhost:3000/api/user-exists${encodeURIComponent(email)}`);
