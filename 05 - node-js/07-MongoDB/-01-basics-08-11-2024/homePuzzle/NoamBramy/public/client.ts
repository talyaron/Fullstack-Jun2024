...
async function getPosts() {
    try {
        const response = await fetch('/api/users/get-posts');
...
async function handleSendForm(event: Event) {
    try {
        ...
        const response = await fetch('/api/users/send-form', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.message || 'Failed to send post data');
        }
        await getPosts();
...
async function updatePost(id: string, title: string, text: string, image: string) {
    try {
        const response = await fetch(`/api/users/update-post/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function handleDeletePost(id: string) {
    try {
        const response = await fetch(`/api/users/delete-post/${id}`, {
            method: 'DELETE'
        });
